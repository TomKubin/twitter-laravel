    ...
    <x-auth-card>
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="flex items-center justify-start mt-4 w-full">
                <div class="w-full">
                    <a class="table underline text-sm text-white hover:text-gray-400" href="{{ route('register') }}">
                            {{ __('Register') }}
                    </a>
                    @if (Route::has('password.request'))
                        <a class="table underline text-sm text-white hover:text-gray-400" href="{{ route('password.request') }}">
                            {{ __('Reset Password') }}
                        </a>
                    @endif
                </div>
